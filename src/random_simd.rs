...
const TOTAL_AMOUNT_VECTORS: usize = 3_000_000;
const VECTORS_PER_BLOCK: usize = 16;

// Total candidates per request. Instead of one 32K contiguous sample, this
// scans four independent 8K contiguous windows. The total amount of distance
// work is unchanged, but the sampled coverage is less dependent on one region.
const VECTOR_SAMPLE: usize = 32_768;
const SAMPLE_WINDOWS: usize = 4;

const TOTAL_BLOCK_SAMPLE: usize = VECTOR_SAMPLE / VECTORS_PER_BLOCK;
const BLOCKS_PER_WINDOW: usize = TOTAL_BLOCK_SAMPLE / SAMPLE_WINDOWS;
const TOTAL_AMOUNT_BLOCKS: usize = TOTAL_AMOUNT_VECTORS / VECTORS_PER_BLOCK;
const BLOCK_SIZE_BYTES: usize = VECTORS_PER_BLOCK * DIMENSIONS * 2;
const BLOCK_CHOICES: usize = TOTAL_AMOUNT_BLOCKS - BLOCKS_PER_WINDOW + 1;
...
impl RandomSIMD {
            ...
            .map(|chunk| {
                ...
                for dim in 0..DIMENSIONS {
                    for lane in 0..VECTORS_PER_BLOCK {
                        ...
                        block[dim][lane] =
                            u16::from_le_bytes(chunk[offset..offset + 2].try_into().unwrap());
                    }
                }
    ...
    #[inline]
    #[target_feature(enable = "avx2")]
    unsafe fn distances_16(
        transaction: &[u16; DIMENSIONS],
        block: &Block,
    ) -> [u32; VECTORS_PER_BLOCK] {
        let mut acc_lo = _mm256_setzero_si256(); // lanes 0..7
        let mut acc_hi = _mm256_setzero_si256(); // lanes 8..15
    ...
    #[inline]
    fn transaction_noise(transaction: &[u16; DIMENSIONS]) -> u64 {
        let mut h = 0x9E37_79B9_7F4A_7C15u64;
    ...
    #[inline]
    fn sample_start_block(base_noise: u64, window_idx: usize) -> usize {
        let window_noise = base_noise
            ^ ((window_idx as u64).wrapping_add(1)).wrapping_mul(0x9E37_79B9_7F4A_7C15);

        Self::fast_bound(Self::mix64(window_noise), BLOCK_CHOICES)
    }

    pub fn fraud_score(&self, transaction: &[u16; DIMENSIONS]) -> f64 {
        let mut best = [(u64::MAX, 0usize); 5];
        let base_noise = Self::transaction_noise(transaction);

        for window_idx in 0..SAMPLE_WINDOWS {
            let start_block = Self::sample_start_block(base_noise, window_idx);
            let end_block = start_block + BLOCKS_PER_WINDOW;

            debug_assert!(end_block <= self.blocks.len());

            let mut global_idx = start_block * VECTORS_PER_BLOCK;

            for block in &self.blocks[start_block..end_block] {
                let distances = unsafe { Self::distances_16(transaction, block) };

                for i in 0..VECTORS_PER_BLOCK {
                    Self::insert_best(&mut best, distances[i] as u64, global_idx + i);
                }

                global_idx += VECTORS_PER_BLOCK;
            }
        }
        ...
        for (_, idx) in best {
            ...
            fraud_count += ((self.labels[word_idx] >> bit_idx) & 1) as usize;
        }
